...
- Added `AnimatableArray`, `AnimatableDictionary`, and `AnimatableProperties` helpers for animating arrays, dictionaries, and nested animatable values. `AnimatableProperties` synthesizes animation data from supplied writable key paths and uses the complete animation data of nested `Animatable` properties.
